...
include "p256.circom";
include "public_key_lookup.circom";

// Multiply an arbitrary scalar by the fixed public key Q using a stride-8 lookup
// table generated offline. The scalar is provided as k registers of n bits each.
template FixedPubkeyMul(n, k) {
    var stride = 8;
    ...
    var powers[num_strides][1 << stride][2][k];
    powers = get_q_pow_stride8_table(n, k);

    var dummyHolder[2][100] = get_q_dummy_point(n, k);
    var dummy[2][k];
    ...
    for (var i = 0; i < k; i++) {
        out[0][i] <== partial[num_strides - 1][0][i];
        out[1][i] <== partial[num_strides - 1][1][i];
    }
}

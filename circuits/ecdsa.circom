...
include "fixed_pubkey_mul.circom";
...
// Optimized verification using a fixed public key. The public key is compiled
// into the circuit via a precomputed lookup table.
template ECDSAVerifyFixedPubkey(n, k) {
    assert(k >= 2);
    assert(k <= 100);
    ...
    component pubkey_mult = FixedPubkeyMul(n, k);
    for (var idx = 0; idx < k; idx++) {
        pubkey_mult.scalar[idx] <== pubkey_coeff.out[idx];
    }
